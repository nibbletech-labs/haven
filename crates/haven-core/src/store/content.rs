...
impl Store {
    ...
    pub fn backlog_markdown(&self, project_key: &str, title: &str) -> Result<String> {
        ...
        let mut open_committed = Vec::new();
        for item in committed {
            if self.is_open_backlog_item(&item)? {
                open_committed.push(item);
            }
        }
        ...
        let mut open_icebox = Vec::new();
        for item in icebox {
            if self.is_open_backlog_item(&item)? {
                open_icebox.push(item);
            }
        }
        ...
        if open_committed.is_empty() {
            let _ = writeln!(out, "_(none)_\n");
        } else {
            for item in &open_committed {
        ...
        if open_icebox.is_empty() {
            let _ = writeln!(out, "_(none)_\n");
        } else {
            for item in &open_icebox {
                out.push_str(&self.backlog_line(item)?);
            }
        }
        Ok(out)
    ...
    fn is_open_backlog_item(&self, item: &Item) -> Result<bool> {
        if item.status == Status::Done {
            return Ok(false);
        }
        if item.node_type.is_container() {
            let (state, _, has_uncommitted) = self.container_rollup(item.id)?;
            if state == RollupState::Done && !has_uncommitted {
                return Ok(false);
            }
        }
        Ok(true)
...
mod tests {
    ...
    fn backlog_markdown_is_deterministic() {
        ...
        s.add_item(
            None,
            NewItem {
                title: "Icebox shipped".into(),
                status: Some(Status::Done),
                ..Default::default()
            },
        )
        .unwrap();
        let finished_phase = s
            .add_item(
                None,
                NewItem {
                    title: "Finished phase".into(),
                    node_type: Some(NodeType::Phase),
                    commit: true,
                    ..Default::default()
                },
        ...
        s.add_item(
            None,
            NewItem {
                title: "Finished phase leaf".into(),
                commit: true,
                status: Some(Status::Done),
                parent: Some(finished_phase.reference.clone()),
                ..Default::default()
            },
        )
        .unwrap();
        ...
        assert!(
            !a.contains("Already shipped"),
            "backlog projection should omit completed history: {a}"
        );
        assert!(
            !a.contains("Icebox shipped"),
            "backlog projection should omit completed icebox items: {a}"
        );
        assert!(
            !a.contains("Finished phase"),
            "backlog projection should omit completed rollup containers: {a}"
        );
        assert!(
            !a.contains("Archived item"),
            "backlog projection should omit dead (archived) committed items: {a}"
        );
        assert!(
            !a.contains("Superseded item"),
            "backlog projection should omit dead (superseded) committed items: {a}"
        );
