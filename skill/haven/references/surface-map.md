...
| `haven_next` | `owner?, limit?` — compact items; `owner` filters ASSIGNMENT (`owner_kind = owner`), unassigned (NULL) excluded. always returns `{items, advisory}`; on a run-shaped ai frontier (≥5 committed-ready ai leaves) `advisory` points at the orchestrate-run skill, and is `null` otherwise (the envelope never varies with the data) |
...
| `haven_graph` | `lineage?, all?, include_acceptance?, node_limit?, edge_limit?, lineage_limit?` — bounded MCP graph read (compact nodes + `{kind,from,to}` edges); each node carries a boolean `has_acceptance` flag (the sealed/unsealed signal) instead of the `done_looks_like` prose — pull the text per-node via `haven_get_item`, or pass `include_acceptance:true` to ride it on the nodes in one read (verify/dispatch); live nodes only unless `all`; defaults/hard caps are 100 nodes, 1000 edges, 250 lineage links, and the response carries `totals`, `omitted`, `limits`, and `truncated`. Edges are capped independently of the node slice, so a node cap never hides structure: endpoints cited but not carried are listed in `dangling_refs`, and a biting edge cap reports `edges_omitted_by_kind`. The CLI `graph` applies the same caps and returns the same envelope — `--full` is the only uncapped export |
...
(live-only unless `all`, plus a boolean `has_acceptance` flag — not the `done_looks_like` prose unless `include_acceptance:true`) and MCP
