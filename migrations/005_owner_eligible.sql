-- HV-66: eligibility-vs-assignment axis on nodes.
--
-- `owner_eligible IN ('human','ai','any')` (NULL = untriaged) is a NEW,
-- additive eligibility axis, DISTINCT from `owner_kind` (the assignment axis,
-- left untouched). `next --owner <k>` filters on eligibility; an untriaged
-- (NULL) item is never auto-pulled.
--
-- SQLite cannot add a CHECK-constrained column in place, so this is the full
-- rename-rebuild of `nodes` (the exact pattern of 003_anchor_type.sql): drop
-- FTS + triggers + indexes, rename `nodes` aside, recreate it with the new
-- column, copy every row preserving integer ids, rebuild the child-FK tables
-- whose foreign keys ALTER...RENAME repointed at `nodes_old`, then recreate
-- indexes, the FTS vtable, and the FTS triggers.
--
-- CRITICAL: the new CREATE and the INSERT...SELECT carry forward EVERY existing
-- column — including `due_at` (added in place at migration 004), `done_looks_like`
-- and `why` — or the rebuild silently drops that data. `owner_eligible` is the
-- only new column; it is absent from the INSERT...SELECT, so existing rows take
-- the column's NULL default (no backfill — eligibility is a deliberate signal).
...
CREATE TABLE nodes (
  ...
  why TEXT,
  due_at TEXT,
  owner_eligible TEXT CHECK (owner_eligible IN ('human','ai','any')),
