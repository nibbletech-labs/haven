-- 003: first-class living-doc anchor nodes.
--
-- SQLite cannot add a value to a CHECK constraint in place. Rebuild `nodes`
-- with the widened type enum, preserve integer ids so all structural references
-- remain valid, then rebuild child tables whose foreign keys were rewritten to
-- `nodes_old` by ALTER TABLE RENAME.
...
INSERT INTO nodes (
  ...
  last_sync_error, last_synced_at, done_looks_like, why
)
SELECT
  id, public_id, project_id, ref, title, body, type, status, owner_kind,
  assignee, wait_state, committed, priority, sort_key, metadata, created_at,
