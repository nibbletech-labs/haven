...
It downloads the x64 release tarball, verifies its sha256, extracts with the tar.exe that ships in Windows 10 1803+, installs `haven.exe` to `%LOCALAPPDATA%\Programs\haven\bin` (override with `HAVEN_BIN_DIR`), adds that directory to your user PATH, and runs `haven setup`. Pin a version with `$env:HAVEN_VERSION = 'v0.2.0'`. Windows on ARM has no prebuilt binary and the installer says so rather than guessing. Already-open terminals keep their old PATH; open a new one to pick up `haven`.
